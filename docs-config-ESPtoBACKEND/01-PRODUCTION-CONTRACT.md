...
Backend membaca raw body, bukan JSON dan bukan multipart. WAV harus RIFF/WAVE, PCM format `1`, mono, `16000 Hz`, `16 bit`, byte rate/block align valid, data tidak kosong, dan durasi maksimal 60 detik. Batas ukuran adalah `3,145,728` byte.
...
| Event | Payload wajib |
|---|---|
| `authenticate` | `event`, `device_id`, `device_token` |
| `audio_playback_done` | `event`, `request_id` UUIDv4 |
| `audio_playback_failed` | `event`, `request_id`, `reason` |

Nilai `reason` hanya: `DOWNLOAD_FAILED`, `DECODE_FAILED`, `PLAYBACK_FAILED`.

## WebSocket outbound dari backend

| Event | Arti dan field |
|---|---|
| `authenticated` | `status="ok"`, `device_id`, `backend_state` (`idle|thinking|audio_ready`), `active_request_id` (string atau null) |
| `authentication_failed` | `error="INVALID_DEVICE_CREDENTIALS"`; setelah itu koneksi ditutup |
| `connection_replaced` | `reason="NEW_CONNECTION_ESTABLISHED"`; koneksi lama harus berhenti dipakai |
| `display_status` | `request_id`, `status="thinking"` |
| `audio_ready` | `request_id`, `audio_url`, `format="mp3"`, `expires_in_seconds` |
| `request_failed` | `request_id`, `code`, `recoverable=true` |
...
8. ESP GET `audio_url` MP3, memvalidasi/decoding, lalu playback.
9. Setelah playback benar-benar selesai, ESP mengirim `audio_playback_done`.
10. Jika download, decode, atau playback gagal, ESP mengirim event failed dengan reason yang tepat.
