...
async function startPipelineFixtures(): Promise<PipelineFixtures> {
  ...
  const hermesServer = http.createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      if (request.method === "POST" && (path === "/v1/responses" || path === "/v1/chat/completions")) {
        const b = JSON.parse(Buffer.concat(chunks).toString("utf8"));
        if (b.stream) {
          hermesRequests.push(b);
          response.writeHead(200, { "content-type": "text/event-stream" });
          response.write("data: " + JSON.stringify({ choices: [{ delta: { content: "Hi! **Joy** is ready to help." } }] }) + "\n\n");
        ...
        response.end(
          JSON.stringify({
            ...
            output: [
              ...
              {
                type: "message",
                content: [{ type: "output_text", text: "Hi! **Joy** is ready to help." }],
              },
            ],
          }),
        );
...
describe("hardware test mode flow", () => {
  ...
  it("supersedes previous audio_ready request when a second request arrives", async () => {
    inbox = await connectDevice(runtime);
    const audioReady = inbox.next("audio_ready");
    await voicePost().expect(202);
    await audioReady;

    const response = await voicePost(secondRequestId).expect(202);
    expect(response.body).toEqual({
      request_id: secondRequestId,
      status: "processing",
    });
    expect(runtime.backend.requestStore.get(requestId)?.status).toBe("failed");
    expect(runtime.backend.requestStore.get(requestId)?.errorCode).toBe("SUPERSEDED");
  });
...
describe("full voice pipeline mode", () => {
  it("runs local STT fixture â†’ Hermes fixture â†’ TTS fixture when hardware test mode is disabled", async () => {
    ...
    try {
      ...
      const download = await request(String(event.audio_url)).get("").expect(200);
      expect(download.headers["content-type"]).toMatch(/^audio\/mpeg/);
      expect(download.body.length).toBeGreaterThanOrEqual(fixtures.mp3Length);
      ...
      expect(fixtures.ttsRequests[0]).toMatchObject({
        request_id: requestId,
      });
      expect(fixtures.ttsRequests.map((r: any) => r.text).join(" ")).toContain("Joy is ready to help.");
    } finally {
