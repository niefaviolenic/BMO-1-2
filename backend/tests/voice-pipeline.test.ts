...
interface CapturedSocketEvent {
  event: string;
  [key: string]: unknown;
}

class FakeSockets {
  readonly events: CapturedSocketEvent[] = [];
...
async function makeRuntime(overrides: {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "joy-p4-"));
  ...
  const service = new VoicePipelineService({
    ...
    hermes: {
      generate: async () => "Hi! Joy is ready to help.",
      ...overrides.hermes,
    },
...
describe("VoicePipelineService", () => {
  it("runs STT → Hermes → TTS, stores MP3, emits audio_ready, and deletes WAV input", async () => {
    ...
    try {
      ...
      expect(result.transcript).toBe("halo joy");
      expect(result.responseText).toBe("Hi! Joy is ready to help.");
      ...
      expect(runtime.sockets.events.map((event) => event.event)).toEqual([
        "display_status",
        "audio_ready",
      ]);
    } finally {
