...
async function runCli(): Promise<void> {
  const selfHost = process.env.FAKE_ESP32_SELF_HOST === "true" || (!process.env.JOY_BASE_URL && !process.env.DEVICE_TOKEN);
  let cleanup: (() => Promise<void>) | undefined;
  let baseUrl = process.env.JOY_BASE_URL ?? "http://127.0.0.1:3000";
  let deviceToken = process.env.DEVICE_TOKEN ?? "";
  if (selfHost) {
    const tempDir = await mkdtemp(join(tmpdir(), "joy-fake-esp32-"));
  ...
  const options: FakeEsp32Options = {
    baseUrl,
    deviceId: process.env.DEVICE_ID ?? "joy-001",
    deviceToken,
