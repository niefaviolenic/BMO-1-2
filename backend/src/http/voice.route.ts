...
export function createVoiceRouter(dependencies: VoiceRouteDependencies): Router {
  ...
  const accept: RequestHandler = async (request, response) => {
    ...
    const activeForDevice = requestStore.getActiveForDevice(context.deviceId);
    if (activeForDevice) {
      const isAudioReady = activeForDevice.status === "audio_ready";
      const isStale = Date.now() - activeForDevice.createdAt > 15_000;
      if (isAudioReady || isStale) {
        requestStore.fail(activeForDevice.requestId, "SUPERSEDED");
      } else {
        response.status(409).json({
          error: "DEVICE_BUSY",
