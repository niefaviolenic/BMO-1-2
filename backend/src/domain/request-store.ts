...
export class RequestStore {
  ...
  constructor(options: RequestStoreOptions = {}) {
    ...
    this.#activeTimeoutMs = options.activeTimeoutMs ?? 20_000;
  }

  create(input: NewRequest): VoiceRequestRecord {
    this.collectGarbage();
    const activeRequestId = this.#activeByDevice.get(input.deviceId);
    if (activeRequestId) {
      const activeRecord = this.#requests.get(activeRequestId);
      const now = this.#now();
      const isAudioReady = activeRecord?.status === "audio_ready";
      const isStale =
        activeRecord &&
        (now - activeRecord.createdAt > 15_000 || now - activeRecord.createdAt > this.#activeTimeoutMs);

      if (!activeRecord || this.#isTerminal(activeRecord) || isAudioReady || isStale) {
        if (activeRecord && !this.#isTerminal(activeRecord)) {
          this.fail(activeRecord.requestId, "SUPERSEDED");
        } else {
          this.#activeByDevice.delete(input.deviceId);
        }
      } else {
  ...
  getActiveForDevice(deviceId: string, now = this.#now()): VoiceRequestRecord | undefined {
    const requestId = this.#activeByDevice.get(deviceId);
    if (!requestId) return undefined;
    const record = this.#requests.get(requestId);
    if (!record || this.#isTerminal(record)) {
      this.#activeByDevice.delete(deviceId);
      return undefined;
    }
    if (now - record.createdAt > this.#activeTimeoutMs) {
      this.#release(record);
      return undefined;
    }
    return record;
