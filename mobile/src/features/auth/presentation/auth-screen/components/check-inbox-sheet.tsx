...
export type CheckInboxContentProps = {
  email: string;
...
export const DUMMY_VALID_CODE = '123456';
...
export function CheckInboxContent({
  ...
  const isIncorrectOrExpired =
    codeError === 'Incorrect code or code has expired.';
  ...
  const handleSubmit = () => {
    ...
    if (!trimmedCode) {
      setCodeError('Please enter the code to continue.');
      return;
    }

    if (trimmedCode !== DUMMY_VALID_CODE) {
      setCodeError('Incorrect code or code has expired.');
      return;
    }
  ...
  return (
    <View style={styles.contentColumn}>
      ...
      <View style={styles.headerGroup}>
        <Text style={[styles.titleText, { color: theme.text }]} testID="check-inbox-title">
          Check Your Inbox
        </Text>
        <Text style={[styles.subtitleText, { color: theme.textSecondary }]} testID="check-inbox-subtitle">
      ...
      <View style={styles.inputSection}>
        ...
        >
          <View style={styles.textColumn}>
            {isCodeFilled && (
              <Text style={[styles.inputLabel, { color: theme.inputPlaceholder }]} testID="code-input-label">
                Code
              </Text>
            )}
            <TextInput
              ...
              placeholder={isCodeFilled ? undefined : 'Code'}
      ...
      <Pressable
        ...
        testID="check-inbox-continue-button"
      >
      ...
      >
        <Text
          style={[
            ...
            countdown > 0 ? styles.resendEmailDisabled : styles.resendEmailActive,
          ]}
        >
          {countdown > 0 ? `Resend Email in ${countdown}s` : 'Resend Email'}
        </Text>
      ...
      <Text style={[styles.footerText, { color: theme.textMuted }]} testID="check-inbox-footer">
...
export type CheckInboxSheetProps = CheckInboxContentProps & {
  isVisible: boolean;
  onClosePress: () => void;
...
export function CheckInboxSheet({
  isVisible,
  onClosePress,
  ...
  return (
    <ModalBottomSheet
      ...
      testID="check-inbox-modal"
      closeButtonTestID="check-inbox-sheet-close-button"
    >
      <CheckInboxContent {...contentProps} />
    </ModalBottomSheet>
  );
}
